...
class ImprovedPositionalEncoding3D(torch.nn.Module):
    def __init__(self, Z, H, W, d_model=32):
        ...
        self.proj_detector = torch.nn.Sequential(
            torch.nn.Conv3d(3, d_model, 1),
            torch.nn.LayerNorm([d_model, Z, H, W]),
            torch.nn.ReLU(),
            torch.nn.Conv3d(d_model, d_model, 1),
        )

        self.proj_sinusoidal = torch.nn.Sequential(
            torch.nn.Conv3d(d_model * 3, d_model, 1),
            torch.nn.LayerNorm([d_model, Z, H, W]),
            torch.nn.ReLU(),
            torch.nn.Conv3d(d_model, d_model, 1),
        )
        ...
        self.final_norm = torch.nn.LayerNorm([d_model, Z, H, W])
    ...
    def forward(self, x):
        ...
        weights = torch.nn.F.softmax(self.combine_weights, dim=0)
