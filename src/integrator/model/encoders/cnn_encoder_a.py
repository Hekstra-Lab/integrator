...
class CNNResNet2(BaseEncoder):
    def __init__(
        ...
        super().__init__()
        self.use_ln = use_ln  # Keeping for backward compatibility
        self.Z = Z
        self.H = H
        self.W = W
        ...
        # Replace GroupNorm with DynamicTanh
        self.norm_counts = DynamicTanh(conv1_out_channel // 2)
        ...
        # Replace GroupNorm with DynamicTanh
        self.norm_pos = DynamicTanh(conv1_out_channel // 2)
        ...
        # Residual blocks - replace GroupNorm with DynamicTanh
        self.layer1 = torch.nn.Sequential(
            torch.nn.Conv3d(
                conv1_out_channel, conv1_out_channel, kernel_size=3, padding=1
            ),
            DynamicTanh(conv1_out_channel),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv3d(
                conv1_out_channel, conv1_out_channel, kernel_size=3, padding=1
            ),
...
class ImprovedPositionalEncoding3D(torch.nn.Module):
    def __init__(self, Z, H, W, dmodel=32):
        ...
        # Balanced projection layers - replace LayerNorm with DynamicTanh
        self.proj_detector = torch.nn.Sequential(
            torch.nn.Conv3d(3, dmodel, 1),
            DynamicTanh(dmodel),
            torch.nn.ReLU(),
        ...
        self.combine_weights = torch.nn.Parameter(torch.ones(2))
        # Replace LayerNorm with DynamicTanh
        self.final_norm = DynamicTanh(dmodel)
...
if __name__ == "__main__":
    ...
    model = CNNResNet2(Z=Z, H=H, W=W, dmodel=dmodel)

    # Forward pass
    output = model(x)

    print(output.shape)  # Should be [batch_size, dmodel]
