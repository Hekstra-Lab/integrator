...
class PixelEncoder(nn.Module):
    def __init__(
        self, encoding_dim=64, depth=10, dmodel=64, dropout=0.0
    ):
        super().__init__()
...
def encode_raw_counts(counts, masks, encoding_dim=64):
    ...
    pixel_pos = torch.arange(num_pixels, device=device).float().unsqueeze(0).expand(batch_size, -1)
...
# class MLP(torch.nn.Module):
    # def __init__(self, width, depth, dropout=None, output_dims=None ):
        # super().__init__()
        # layers = [
            # ResidualLayer(width, dropout=dropout) for _ in range(depth)
        # ]
...
# class IntegratorMLP(BaseIntegrator):
    ...
    # def forward(self, counts, shoebox, metadata, masks, reference):
        ...
        # #shoebox [batch_size, pixels, features]
        # rep = self.encoder(shoebox)
