...
class Loss(torch.nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.device = device
        self.eps = torch.nn.Parameter(torch.tensor(eps, device=self.device), requires_grad=False)
        self.beta = torch.nn.Parameter(torch.tensor(beta, device=self.device), requires_grad=False)
        self.p_I_scale = torch.nn.Parameter(torch.tensor(p_I_scale, device=self.device), requires_grad=False)
        self.p_bg_scale = torch.nn.Parameter(torch.tensor(p_bg_scale, device=self.device), requires_grad=False)
        self.p_p_scale = torch.nn.Parameter(torch.tensor(p_p_scale, device=self.device), requires_grad=False)
    ...
    def forward(self, rate, counts, q_I, q_bg, q_p, dead_pixel_mask, eps=1e-5):
        ...
        p_p = self._move_distribution_to_device(torch.distributions.dirichlet.Dirichlet(torch.ones(3 * 21 * 21)))
