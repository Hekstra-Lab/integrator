...
integrator.predict
        |
        v
pred.parquet / test_preds_all.parquet / MTZ outputs / diagnostics
```
...
| Transform control | Supports MFX-friendly transforms such as `standardization` or `none`. |
...
MFX pixels can be float and negative, so Anscombe-style transforms are unsafe. The MFX path uses raw/standardization-style handling instead.

### 3. `src/integrator/model/loss/wilson_loss.py`

Updated the Wilson loss to support MFX/Jungfrau pixel behavior and alternative `qi` posterior distributions.
...
### 4. `src/integrator/model/distributions/lognormal.py`
...
### 5. `src/integrator/model/distributions/folded_normal.py`
...
Use separate environments for shoebox extraction and training.

### Shoebox extraction environment
...
- Should FoldedNormal be the next candidate for a larger MFX run?
- What architecture changes would make the model more appropriate for SFX/MFX still images?
- Which diagnostics are most useful before scaling beyond the 25-file smoke test?
