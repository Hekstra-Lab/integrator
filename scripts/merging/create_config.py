...
# columns -> the diagnose_merging VARIANT names to run per checkpoint.
_COLUMN_VARIANTS = {
    "amplitude": ["F_noFW"],
    "intensity": ["I_FW"],
    "both": ["F_noFW", "I_FW"],
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Where per-checkpoint outputs land (default: <run_dir>/ckpt_eval).",
    )
    p.add_argument(
        "--chi2-inflation",
        action="store_true",
...
def main():
    ...
    phenix_env = _resolve(args.phenix_env, None, "PHENIX_ENV", "phenix_env")
