...
posterior `q(I_h)` from the model's buffer, writes an anomalous MTZ
with both intensity and amplitude columns, and runs phenix.refine in
three configurations to disentangle whether the model's I or F is the
better input and whether French-Wilson scaling helps:

    F_noFW : F(+),SIGF(+),... as amplitudes      (FW unused)
    I_FW   : I(+),SIGI(+),... as intensities     (french_wilson_scale=True)
    I_noFW : I(+),SIGI(+),... as intensities     (french_wilson_scale=False)

For each variant, runs `rs.find_peaks` for anomalous peaks.
Works for ConjugateMergingIntegrator, DeepSetsMergingIntegrator, and
AmortizedMergingIntegrator (all expose `get_merged_qi()` returning a
per-HKL Gamma distribution).
...
Outputs land in RUN_DIR/diagnostics/:
    ...
    I_noFW/{phenix.eff, refine_*, peaks.csv}
    summary.txt
"""
...
def find_last_checkpoint(meta: dict) -> Path:
    ...
    Lightning saves it in `<wandb_log_dir>/checkpoints/last.ckpt` (or as a
    symlink). Fallback: the most recent `epoch=*.ckpt` in that dir.
    """
...
def load_integrator(cfg: dict, checkpoint_path: Path):
    ...
    # skip_warmstart: the checkpoint supplies the trained profile basis, so the
    # warmstart .pt is redundant (and may be absent when diagnosing off-cluster).
...
def compute_sigma_inflation_per_hkl(
    ...
    metadata = torch.load(
        metadata_path, weights_only=False, map_location="cpu"
    )

    for key in ("asu_id", "intensity.prf.value"):
        if key not in metadata:
            raise KeyError(
                f"metadata.pt missing '{key}' - cannot compute inflation. "
                "Skip with --no-chi2-inflation."
            )
...
# Three variants. Each tuple: (variant name, labels, *array_type, fw flag).
VARIANTS = [
...
def main():
    ...
    eff_template_path = Path(cfg["output"]["phenix_eff"])
    if not eff_template_path.exists():
        logger.error("phenix.eff template not found at %s", eff_template_path)
        sys.exit(1)
    ...
    for variant, labels, star_token, fw in VARIANTS:
        ...
        eff_text = render_eff(template, mtz_path, labels, star_token, fw)
        eff_path.write_text(eff_text)
        logger.info("Wrote %s", eff_path)

        if args.skip_phenix:
            summary_lines.append(f"[{variant}] eff written (phenix skipped)")
            continue
