...
#SBATCH -o mono_train_%j.out
#SBATCH -e mono_train_%j.err
#
# Monochromatic training on the scavenger partition instead of `gpu`.
...
run_dir="$OUT/${RUN_NAME:-mono_hewl9b7c_${SLURM_JOB_ID}}"
mkdir -p "$run_dir"
...
# the partition caps cores per GPU, so the worker count has to follow the request
