#!/bin/bash
# One command for the whole mono chain: preflight -> train -> predict/DIALS/
# phenix/peaks/merging_stats -> per-run figures -> comparison plots.
#
# Each stage is its own SLURM job, chained with --dependency=afterok, because
# training wants its own GPU allocation for hours and a single job would hold a
# GPU idle through DIALS/phenix. Submitting the chain up front means one command,
# everything queued, each stage starting when its input exists, and a failure
# stopping the rest instead of cascading garbage.
#
#   ./run_all.sh                          # the full chain, new run
#   ./run_all.sh --from pipeline          # training already done
#   ./run_all.sh --from figures --run-name mono_hewl9b7c
#   ./run_all.sh --only plots
#   ./run_all.sh --dry-run                # print the sbatch calls, submit nothing
#
# Options:
#   --run-name NAME   run directory under $OUT (default: mono_hewl9b7c_<date>)
...
#   --config FILE     training config (default configs/mono/hewl9b7c_mono.yaml)
#   --process-cfg F   downstream DIALS/phenix config (default from env.sh)
...
DRY=0
PREFLIGHT=1
RUN_NAME="${RUN_NAME:-mono_hewl9b7c_$(date +%Y%m%d)}"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --process-cfg)   PROCESS_CFG="$2"; shift 2;;
...
if [[ "$PREFLIGHT" == "1" && "$DRY" == "0" && "$from_i" -eq 0 ]]; then
    ...
    python "$INTEGRATOR_ROOT/scripts/preflight.py" --config "$CONFIG"
    echo
fi
...
if [[ "$from_i" -le 1 && "$to_i" -ge 1 ]]; then
    jobs[pipeline]=$(submit pipeline "$dep" "${GPU_ARGS[@]}" \
        -J pipeline -o "pipeline_%j.out" -e "pipeline_%j.err" \
        --wrap "cd $KIT_DIR && RUN_DIR=$run_dir PROCESS_CFG=$PROCESS_CFG ./03_pipeline.sh")
    dep="${jobs[pipeline]}"
fi
