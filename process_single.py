...
def run_phenix(phenix_env, mtz_file, phenix_eff):
...
def update_phenix_eff(phenix_eff_template, updated_phenix_eff, mtz_file):
    with open(phenix_eff_template, "r") as file:
...
def process_single_refl(refl_file, expt_file, dials_env, phenix_env, phenix_eff):
...
if __name__ == "__main__":
    ...
    if file_index >= len(refl_file_paths):
        print(f"Error: file_index {file_index} out of range. Only {len(refl_file_paths)} files available.")
        sys.exit(1)
