    ...
    def setup(self, stage=None):
        ...
        if shoebox_features is not None:
            self.full_dataset = TensorDataset(
                shoeboxes, metadata, dead_pixel_mask, shoebox_features, counts
            )
        else:
            self.full_dataset = TensorDataset(
...
class CrystalNorm(nn.Module):
    def __init__(self, num_channels):
        ...
        self.coord_norm = nn.GroupNorm(
            1, num_channels // 4 * 3, affine=False
        )  # 3/4 of channels for coords
        self.intensity_norm = nn.GroupNorm(
            1, num_channels // 4, affine=False
...
class CNNResNet(torch.nn.Module):
    def __init__(
        self,
        use_ln=True,  # Changed from use_bn
        conv1_in_channel=6,
        ...
        if self.use_ln:
            self.ln1 = nn.GroupNorm(2, conv1_out_channel, affine=False)

        self.relu = torch.nn.ReLU(inplace=True)
        self.maxpool = torch.nn.MaxPool2d(kernel_size=3, stride=1, padding=0)

        self.layer1 = self._make_layer(
            conv1_out_channel, conv1_out_channel, layer1_num_blocks, use_ln=self.use_ln
        )

        self.avgpool = torch.nn.AdaptiveAvgPool2d((1, 1))

    def _make_layer(self, in_channels, out_channels, num_blocks, stride=1, use_ln=True):
        """Helper method to create a layer of residual blocks"""
        layers = []
        layers.append(Residual(in_channels, out_channels, strides=stride))
        for _ in range(1, num_blocks):
            layers.append(Residual(out_channels, out_channels))
        return torch.nn.Sequential(*layers)

    def verify_output(self, x, stage_name):
        if torch.isnan(x).any():
            raise ValueError(f"NaN detected in {stage_name}")
        if torch.isinf(x).any():
            raise ValueError(f"Inf detected in {stage_name}")
        return x
    ...
    def reshape(self, x, mask):
        ...
        Z_coords = x[..., 2][..., : self.H * self.W].view(x.size(0), 1, self.H, self.W)

        x = torch.cat([X_coords, Y_coords, Z_coords, counts_], dim=1)
        return x

    def forward(self, x, mask, shoebox_data=None):
...
class ResidualLayer(nn.Module):
    def __init__(self, width, dropout=None, use_norm=True):  # Changed parameter name
        ...
        if self.use_norm:
            ...
            self.norm1 = nn.GroupNorm(1, width, affine=False)  # num_groups=1
        self.fc2 = Linear(width, width)
        if self.use_norm:
            self.norm2 = nn.GroupNorm(1, width, affine=False)
...
class ImprovedDetectorPositionalEncoding3D(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(3):
            ...
            coords.append(coord_norm * self.s0cale_factors[i])
...
class Loss(torch.nn.Module):
    def __init__(
        ...
        # p_I=torch.distributions.normal.Normal(torch.tensor(0.0),torch.tensor(1.0)),
        p_p_scale=0.0001,
        p_bg_scale=0.0001,
        p_I_scale=0.0001,
        mc_samples=100,
...
def main():
    ...
    checkpoint_path = "/n/holylabs/LABS/hekstra_lab/Users/laldama/integratorv2/integrator/dials_output/epoch_19/checkpoint.ckpt"

    #  /n/holylabs/LABS/hekstra_lab/Users/laldama/integratorv2/integrator/lightning_logs/version_57300009/checkpoints/epoch=19-step=107260.ckpt"
    ...
    torch.save(final_outputs, "model2_all_preds.pt")
    print("Done!")
